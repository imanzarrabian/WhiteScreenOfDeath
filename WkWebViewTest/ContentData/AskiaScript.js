/*! AskiaScript 1.2.0 Copyright Askia (C) 1994-2012 */﻿/*global ActiveXObject, AskiaScript, replace */
...
XmlLoader.prototype.onreadystatechange=function(){/* Implemente event here */};﻿/*global ActiveXObject, AskiaScript, eXmlState, FOLDER_PLUGINS, FOLDER_PLUGINS, FILENAME_SETTINGS_XML, isNS6, UNDEFINED, replace */
/*jshint evil:true,bitwise:false,proto:true,nomen:false,strict:false,globalstrict:false,onevar:false,white:false,plusplus:false,eqeq:false*/
/*----------------------------------------------------------------------------\
...
|-----------------------------------------------------------------------------|
| Created 2006-05-23 | All changes are in the log above. | Updated 2013-10-31 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
|Askia - AskiaScript - API Version 1.2.0		Copyright Askia © 1994-2013   |
\---------------------------------------------------------------------------- */
...
var AskiaLibrary = {
	...
	JQUERY_UI_DATEPICKER: 'jquery-ui-datepicker.js',
	// Chapters menu
	CHAPTERS_MENU: 'ChaptersMenu.js'
};
...
var AskiaLibraryRevision = {
	...
	JS: {
		//Version 1.1
		"1.1": { CALENDAR: 10, SLIDER: 2, TOOLTIPTEXT: 2, COMMON: 10, RANKING: 5, FLASH: 9, PLUGINS: 4, FILTER_BOX: 6, VALIDATOR: 10, JQUERY: 9, FILTER_BOX: 9 }
	},
	//CSS library
	CSS: {},
...
var AskiaCSS = {
	...
	RANKING: 'Ranking.css',
	// Chapters menu
	CHAPTERS_MENU: 'ChaptersMenu.css',
	// Flash fix
	FLASH_FIX: 'FlashFix.css',
	// Media capture
	MEDIA_CAPTURE: 'MediaCapture.css',
	// jQuery.ui.datepicker
	JQUERY_UI_DATEPICKER: 'jquery-ui-datepicker.css'
};
...
var AskiaScript = {
	...
	load: function (version, local) {
		...
		/*for (var lib in AskiaLibrary) {
			...
			if (lib === AskiaLibrary.CHAPTERS_MENU && version < 1.2) continue;
		...
		/*for (var lib in AskiaCSS) {
			if ((lib === AskiaCSS.CHAPTERS_MENU || lib === AskiaCSS.MEDIA_CAPTURE) && version < 1.2) continue;
		...
		//Add css
		document.write('<link rel="stylesheet" href="' + this.pathOfScripts + 'AskiaScript.css">');
	},
...
|-----------------------------------------------------------------------------|
| Created 2006-05-26 | All changes are in the log above. | Updated 2011-05-16 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
|WebProd - FieldValidator - API Version 1.1.10	Copyright Askia © 1994-2011   |
\----------------------------------------------------------------------------*/
...
Validator.prototype._validateNumber=function(isOnSubmit){
	...
	if (!isOnSubmit && response.slider)return true;   //Don't validate the slider on the fly
...
Validator.prototype._validateDate=function(isOnSubmit){
	...
	/**
		* Fix with regional settings, too many different format to manage,
		* we don't validate the date in the client side anymore
		*/	
//		if (!this._isDate(date)){
//				var sDateFormat = (eDateFormat && eDateFormat.defaultFormat) ? eDateFormat.defaultFormat : ((AskiaScript.regional && AskiaScript.regional.dateFormat) ? AskiaScript.regional.dateFormat : 'MM/dd/yyyy');
//				ErrorStack.add(question,ErrorMessages.DATE_FORMAT_INVALID.replace(/(dd\/MM\/yyyy|jj\/MM\/aaaa)/gi, sDateFormat));
//				return false;
//			}
//			
//		//Don't validate the minimum and the maximum 
//		//if the question is not the questiond date
//		if (question.type!=eQuestionType.date)return true;
//										
//		//Verify the minimum and the maximum
//		var isMinOk=true;
//		if (question.min)isMinOk=(compareDate(getDateFromString(response.htmlControl.value),getDateFromString(question.min))==-1)?false:true;
//		
//		var isMaxOk=true;			
//		if (question.max)isMaxOk=(compareDate(getDateFromString(response.htmlControl.value),getDateFromString(question.max))==1)?false:true;
//		
//		//Format the error messages according to min and max
//		if (!isMinOk || !isMaxOk){
//				var message=ErrorMessages.DATE_VALUE_INVALID;
//				if (question.min && !question.max)message=ErrorMessages.DATE_MINIMUM_INVALID;
//				if (question.max && !question.min)message=ErrorMessages.DATE_MAXIMUM_INVALID;
//				ErrorStack.add(question,message);
//				return false;
//			}
//		
...
Validator.prototype._isDate=function(date){		
	...
	// Search in calendar.js or from the AskiaScript
	var sFormat = (eDateFormat && eDateFormat.defaultFormat) ? eDateFormat.defaultFormat : ((AskiaScript.regional && AskiaScript.regional.dateFormat) ? AskiaScript.regional.dateFormat : 'MM/dd/yyyy');
...
| 2006-05-24 |V 1.0.0														  |
|			 |+ Create calendar												  |
|			 |+ Can set the minimum, maximum and default values				  |
|			 |+ Can translate the shortcut of day							  |
|-----------------------------------------------------------------------------|
| 2007-06-07 |V 1.0.1														  |
|			 |+ Fix: Correct the default date according to min & max		  |
|-----------------------------------------------------------------------------|
| 2008-09-16 |V 1.0.2														  |
|			 |+ Fix: Selection of date in calendar                  		  |
|-----------------------------------------------------------------------------|
| 2009-10-20 |V 1.1.1														  |
|			 |+ Fix: Change the default format of date: MM/dd/yyyy instead    |
|			 |of dd/MM/yyyy                  								  |
|-----------------------------------------------------------------------------|
| 2010-06-02 |V 1.1.5														  |
|			 |+ Fix: Wait again 1/2 second before to lost focus				  |
|-----------------------------------------------------------------------------|
| 2011-02-21 |V 1.1.9														  |
|			 |+ Fix the problem with min and max (make a double check on month|
|-----------------------------------------------------------------------------|
| 2011-05-16 |V 1.1.10														  |
|			 |+ Add regional settings for date and number					  |
|-----------------------------------------------------------------------------|
| 2011-10-24 |V 1.2.0														  |
|			 |+ Read the response using a given format and the settings with  |
|            |   the inner default format (dd/MM/yyyy)                        |
|-----------------------------------------------------------------------------|
| Created 2006-05-24 | All changes are in the log above. | Updated 2011-10-24 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
|WebProd - Calendar - API Version 1.2.0			Copyright Askia © 1994-2011   |
\----------------------------------------------------------------------------*/

/* ===
	Enumerator with the format of date
=== */
var eDateFormat={
		day		: 'dd',
		month   : 'MM',
		year	: 'yyyy',
		defaultFormat:  (AskiaScript.regional && AskiaScript.regional.dateFormat) ? AskiaScript.regional.dateFormat : 'MM/dd/yyyy'
	};
/* ===
	Regular expression to search day/month and year in the date format
=== */
var reDay = /d{1,4}/g, reMonth = /M{1,4}/g, reYear = /y{1,4}/g;

/* ===
	Enumerator with the id calendar control
=== */
var eCalendarElementId={
		container	: 'contCal',
		table		: 'tblCal',
		tdOfDay		: 'tdDay',
		cmbMonth	: 'cmbMonth',
		cmbYear		: 'cmbYear',
		buttonOpen	: 'cmdCalOpen'
	};
	
/* ===
	Enumerator with the className use in calendar
=== */
var eCalendarClassName={
		day				: 'calendar-day',
		dayToDay		: 'calendar-daytoday',
		daySelected		: 'calendar-selectedday',
		dayDisabled		: 'calendar-disabledday',
		dayLetter		: 'calendar-dayletter',
		calBody			: 'calendar-body',
		calTable		: 'calendar-table',
		comboMonth		: 'calendar-combo-month',
		comboYear		: 'calendar-combo-year'
	};
		
/* ===
	Enumerator with letter of month
=== */
var eDayLetterTranslation={
		monday		: 'M',
		tuesday		: 'T',
		wednesday	: 'W',
		thursday	: 'T',
...
/* ===
	Enumerator with the id of month in the eDayLetterTranslation enum
===*/
var eDayLetterByIndex =Array('monday','tuesday','wednesday','thursday','friday','saturday','sunday');

/* ===
	Enumerator for the translation of Month
=== */
var eMonthTranslation={
		january		: 'January',
		february	: 'February',
		march		: 'March',
		april		: 'April',
...
/* ===
	Enumerator with the id of month in the eMonthTranslation enum
===*/
var eMonthByIndex=Array('january','february','march','april','may','june','july','august','september','october','november','december');
	
	
/* ===
	Usefull functions
=== */
//Return string which represent a date of arg
function getDateToString(date){
		...
		var fDate = eDateFormat.defaultFormat,
			
			fDay = fDate.match(reDay)[0],
			fMonth = fDate.match(reMonth)[0],
			fYear = fDate.match(reYear)[0],
			
			sDay = "" + ((fDay.length === 1)?date.getDate():formatNumber(date.getDate())),
			sMonth = "" + ((fMonth.length === 1)?(date.getMonth() + 1):formatNumber(date.getMonth() + 1)),
			sYear = "" + ((fYear.length < 3)?formatNumber(date.getYear()):date.getFullYear());
		
		fDate = fDate.replace(reDay, sDay);
		fDate = fDate.replace(reMonth, sMonth);
		fDate = fDate.replace(reYear, sYear);

		return fDate;		
	}
	
//Return date using a string
function getDateFromString(sDate, useInternalFormat){
		if (sDate=="")return new Date();
		

		var fDate = (!useInternalFormat) ? eDateFormat.defaultFormat : "dd/MM/yyyy",
			
			pDay = fDate.indexOf('d'),
			lDay = fDate.match(reDay)[0].length,
			sDay = sDate.substr(pDay, lDay),
			nDay = parseInt(sDay, 10),
			
			pMonth = fDate.indexOf('M'),
			lMonth = fDate.match(reMonth)[0].length,
			sMonth = sDate.substr(pMonth, lMonth),
			nMonth = parseInt(sMonth, 10) - 1,
			
			pYear = fDate.indexOf('y'),
			lYear = fDate.match(reYear)[0].length,
			sYear = sDate.substr(pYear, lYear),
		...
		if (sYear.length > 2) {
			...
			dt.setFullYear(1000 + nYear);
		} else {
			dt.setFullYear(2000 + nYear);
		}
		return dt;
	}
...
/* ===
	Manage the collection of calendar
=== */
var CalendarHandler={
		//Collections of calendar
		all			:	{},	//Retrieve calendar using id
		childNodes	:	[], //Retrive calendar using index
		lastSelection : null,	//Latest selected calendar
		
		//Add calendar into collections
		add			:	function(oCalendar){
				this.all[oCalendar.id]=oCalendar;
				this.childNodes[this.childNodes.length]=this.all[oCalendar.id];
				return this.all[oCalendar.id];
			},
		//Re-init the values into calendar
		_loadValues	: function(sId){
				var sId=sId.replace(eCalendarElementId.cmbYear,'');
					sId=sId.replace(eCalendarElementId.cmbMonth,'');
				if (!CalendarHandler.all[sId])return;
				CalendarHandler.all[sId]._loadValues();
			},
		//Set the value into calendar
		_setValue	:function(htmlElt){
		        if (!htmlElt)return;
				var cal=htmlElt._calendar;
				cal._setValue(htmlElt.innerHTML);
				return false;
			},
		//Set the status of focus
		_getFocus	:function(sId){
				sId +=''; //Transform sId to string if there is a number
				sId=sId.replace(eCalendarElementId.cmbYear,'');
				sId=sId.replace(eCalendarElementId.cmbMonth,'');
				if (!CalendarHandler.all[sId])return;
				CalendarHandler.lastSelection=CalendarHandler.all[sId];
				CalendarHandler.all[sId]._hasFocus=true;
				CalendarHandler.all[sId].onfocus();
			},
		//Set the status of focus
		_lostFocus	:function(sId){
				setTimeout("CalendarHandler._lostFocus('" + sId + "');",500);
			},
		_lostFocusAfter : function(sId){
				sId +=''; //Transform sId to string if there is a number
				sId=sId.replace(eCalendarElementId.cmbYear,'');
				sId=sId.replace(eCalendarElementId.cmbMonth,'');
				if (!CalendarHandler.all[sId])return;
				CalendarHandler.lastSelection=CalendarHandler.all[sId];
				CalendarHandler.all[sId]._hasFocus=false;	
				setTimeout("CalendarHandler.lastSelection._raiseEvent_onBlur();",100);		
			}
	};
/* ===
	Calendar object
=== */
// Constructor
function Calendar(id,minDate,maxDate,defaultDate){
		//Id of calendar
		this.id =id;				
		...
		if (minDate) {
			this.minDate = getDateFromString(minDate, true);
		}
		...
		this.maxDate.setFullYear(2032);
		if (maxDate) {
			this.maxDate=getDateFromString(maxDate, true);
		}

		//Default value
		this.date=new Date();
		if (defaultDate) {
			this.date=getDateFromString(defaultDate, true);
		}
		//Correct the default date
		if (compareDate(this.date, this.minDate) === -1 ) {
			this.date.setMonth(0);
			this.date.setDate(1);
			this.date.setFullYear(1900);
		}
		if (compareDate(this.date, this.maxDate) === 1) {
			this.date.setMonth(11);
			this.date.setDate(31);
			this.date.setFullYear(2032);
		}

		//Define if calendar is visible or not
		this.isVisible=false;
		
		//Define if the calendar have the focus
		this._hasFocus=false;
		
		//Create a popup window for the calendar
		this._createCalendar();
		
		//Add this calendar into the handler
		CalendarHandler.add(this);
	}

// Show the calendar
Calendar.prototype.show=function(left,top){
		//Set the latest selected calendar
		CalendarHandler.lastSelection=this;
		
		//Load the values
		this._loadValues();
		
		//Show the calendar
		var oDiv=document.getElementById(eCalendarElementId.container + this.id);
			if (left)oDiv.style.left=left + "px";
			if (top)oDiv.style.top=top + "px";
			oDiv.style.display="block";		
					
		//Fix from 1.0.2 removes the focus on combo of month because the lost focus is fires before the onclick
		this.isVisible=true;
		this._hasFocus=true;
	};
// Hide the calendar
...
Calendar.prototype._createCalendar=function(){
	//Create an hidden DIV elements
	var oDiv=document.createElement("DIV");
		oDiv.id=eCalendarElementId.container + this.id;
		oDiv.style.position="absolute";
		oDiv.style.zIndex=1000;
		oDiv.style.display="none";
		oDiv.className=eCalendarClassName.calBody;
		oDiv.innerHTML=this._getHtmlCalendar();
		if (!isIE)
			document.body.appendChild(oDiv);
		else 
			document.write(oDiv.outerHTML);
	//Load values into calendar
	this._loadValues();
};
	
// Return html string of calendar
Calendar.prototype._getHtmlCalendar=function(){

	//Create the string for the letter of day
	var strDayLetter=new StringBuilder();
	for (var i=0;i<eDayLetterByIndex.length;i++){
			strDayLetter.append('<td class="' +  eCalendarClassName.dayLetter + '">' + eDayLetterTranslation[eDayLetterByIndex[i]] + '</td>');
		}


	//Create string for the select box of month
	var strCmbMonth=new StringBuilder();
		strCmbMonth.append('<select onblur="CalendarHandler._lostFocus(this.id);" onfocus="CalendarHandler._getFocus(this.id);" class="' + eCalendarClassName.comboMonth + '" id="' + eCalendarElementId.cmbMonth + this.id + '" onchange="CalendarHandler._loadValues(this.id)">');
		var nMonth=this.date.getMonth();
		for (var i=0;i<eMonthByIndex.length;i++){
				var sSelected=(i==nMonth)?" selected":"";
				strCmbMonth.append('<option value="' + i + '"' +  sSelected + '>' + eMonthTranslation[eMonthByIndex[i]] + '</option>');
			}
		strCmbMonth.append('</select>');
	
	//Create string for the select box of year
	var strCmbYear=new StringBuilder();
		strCmbYear.append('<select onblur="CalendarHandler._lostFocus(this.id);" onfocus="CalendarHandler._getFocus(this.id);" class="' +  eCalendarClassName.comboYear  + '" id="' + eCalendarElementId.cmbYear + this.id + '" onchange="CalendarHandler._loadValues(this.id)">');
		var nYear=this.date.getFullYear();
		for (var i=this.minDate.getFullYear();i<=this.maxDate.getFullYear();i++){
				var sSelected=(i==nYear)?" selected":"";
				strCmbYear.append('<option  value="' + i + '"' +  sSelected + '>' + i + '</option>');
			}
		strCmbYear.append('</select>');
		
	var strBuilder=new StringBuilder();
	strBuilder.append('<table border="0" cellspacing="0" cellpadding="0">');
	strBuilder.append('		<tr>');
	strBuilder.append('			<td>' + strCmbMonth.toString()  + '</td>');
	strBuilder.append('			<td align="right">' + strCmbYear.toString() + '</td>');
	strBuilder.append('		<tr>');
	strBuilder.append('			<td colspan="2">');
	strBuilder.append('		<table class="' + eCalendarClassName.calTable + '" border="0" cellspacing="0" cellpadding="0" width="100%">');
	strBuilder.append('		<tr>');
	strBuilder.append(			strDayLetter.toString());
	strBuilder.append('		</tr>'); 	
	strBuilder.append('		<tr>');
	var cellInRow=0;
	for (var cell=0;cell<42;cell++){
			...
			strBuilder.append('<td onclick="CalendarHandler._setValue(this);" id="' + eCalendarElementId.tdOfDay  + this.id + cell + '"></td>');
			cellInRow ++;
		}
	strBuilder.append('		</tr>');
...
//Load all values in popup of calendar
Calendar.prototype._loadValues=function(){
	//Clear the items of calendar
	for(var i=0;i<42;i++) {
			var elt=document.getElementById(eCalendarElementId.tdOfDay + this.id + i);
				elt.innerHTML = "";
				elt.className=eCalendarClassName.dayDisabled;
				elt._calendar=this;
				//elt.onclick=CalendarHandler._setValue;
		}
	
	//Initialize index for the first day in month
	var myDate=new Date();
	myDate.setDate(1);
	myDate.setMonth (parseInt(document.getElementById(eCalendarElementId.cmbMonth + this.id ).value));
	myDate.setFullYear (parseInt(document.getElementById(eCalendarElementId.cmbYear + this.id).value));
		
	//Initialize the month will be treated
	var currentMonth=myDate.getMonth();
	//Initialize the index of cell
	var currentCell=(myDate.getDay()==0)?6:myDate.getDay()-1;
	//Initialize the date today
	var toDay=new Date();
		
	//While Month is always the same ...
	while(currentMonth == myDate.getMonth()) {
		var elt=document.getElementById(eCalendarElementId.tdOfDay + this.id + currentCell);
			elt.innerHTML=myDate.getDate();
		var sClassName=eCalendarClassName.day;
			
		//Date to day
		if (myDate.toDateString()==toDay.toDateString())sClassName=eCalendarClassName.dayToDay;
		//Date selected
		if (myDate.toDateString()==this.date.toDateString())sClassName=eCalendarClassName.daySelected;
			
		//Date disabled
		if (compareDate(myDate,this.minDate)==-1)sClassName =eCalendarClassName.dayDisabled;
		if (compareDate(myDate,this.maxDate)==1)sClassName =eCalendarClassName.dayDisabled;
			
		elt.className=sClassName;
...
//Set value into calendar
Calendar.prototype._setValue=function(day){
	if (!day)return;
	day=parseInt(day);
	if (isNaN(day))return;
	...
	//Check if date is available accoding to min & max	
	if (compareDate(myDate,this.minDate)==-1)return;
	if (compareDate(myDate,this.maxDate)==1)return;
		
	//Set value in current object
	this.date.setFullYear(myDate.getFullYear());
	this.date.setMonth(myDate.getMonth());
	this.date.setDate(myDate.getDate());
...
//Send the onBlur event
Calendar.prototype._raiseEvent_onBlur=function(){
	if (!this._hasFocus)this.onblur();
};
//Event
Calendar.prototype.onchange=function(){/* RaiseEvent */};
Calendar.prototype.onfocus=function(){/*RaiseEvent*/};
Calendar.prototype.onblur=function(){/*RaiseEvent*/};﻿/*global ActiveXObject, AskiaScript, eQuestionType, eAskiaClass, QuestionHandler,  replace */
/*jshint evil:true,bitwise:false,proto:true,nomen:false,strict:false,globalstrict:false,onevar:false,white:false,plusplus:false,eqeq:false*/
/*----------------------------------------------------------------------------\
|							Askia Slider Objects							  |
|-----------------------------------------------------------------------------|
|                          Created by Askia Company							  |
|							(http://www.askia.com)							  |
|-----------------------------------------------------------------------------|
| Display a slider instead of text-box or combo-box							  |
|-----------------------------------------------------------------------------|
|							Copyright Askia © 1994-2007						  |
|-----------------------------------------------------------------------------|
| Dependencies:																  |
...
|	+ Common.js																  |
|	+ ToolTipText.js														  |
|	+ Skin (css files associated with the skin of slider)					  |
|-----------------------------------------------------------------------------|
| 2006-06-05 |V 1.0.0														  |
|			 |+ First version												  |
|			 |+ Load the slider of numeric question							  |
|			 |+ Load the slider of single question							  |
|-----------------------------------------------------------------------------|
| 2007-09-03 |V 1.0.1														  |
|			 |+ Fix:Verify that the css is applied before to create the slider|
|-----------------------------------------------------------------------------|
| 2009-10-27 |V 1.1.2														  |
|			 |+ Fix:Verify that tooltip exist before to use it				  |
|-----------------------------------------------------------------------------|
| Created 2006-06-05 | All changes are in the log above. | Updated 2009-10-27 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
|WebProd - Slider - API Version 1.1.2			Copyright Askia © 1994-2009   |
\----------------------------------------------------------------------------*/

/* ===
	Enumerator
===	*/
//Available skin into slider
var eSkinName={	
		blueAcier		: 'BlueAcier',
		blueCurve		: 'BlueCurve',
		blueCurveScroll	: 'BlueCurveScroll',
		classical		: 'Classical',
		luna			: 'Luna',
		smoothStreak	: 'SmoothStreak',
		winXp			: 'WinXp'		
	};
...
/* ===
	SlidersHandler. Manage the collections of sliders
=== */
var SlidersHandler={
	//Collections of sliders
	all			: {},
	childNodes	: [],
	lastSelection	: null,
	...
	//Cancel or not the click event
	_cancelClickEvent	: false,
	
	//Method to add slider into collections
	add			: function (oSlider){
			//Don't add the duplicate slider
			if (this.all[oSlider.id])return this.all[oSlider.id];
			
			//Add into collections
			this.all[oSlider.id]=oSlider;
			this.childNodes[this.childNodes.length]=this.all[oSlider.id];
			
			return this.all[oSlider.id];
		},
	//Set the focused on slider
	onmouseover		: function(e){
			if (this.slider==SlidersHandler.lastSelection){
					SlidersHandler.lastSelection.showToolTip();
					return;
				}
			//Set the focus on slider
			SlidersHandler.lastSelection=this.slider;
			//Apply the class of focused
			SlidersHandler.lastSelection.cursor.className = eSliderClassName.cursorOver;	
			//Display the tool tip text
			SlidersHandler.lastSelection.showToolTip();
		},
	//The slider lost the focus
	onmouseout		: function(e){
			if (!SlidersHandler.lastSelection)return;			
			//Change the style of not focused
			SlidersHandler.lastSelection.cursor.className = eSliderClassName.cursor;		
			//Hide the tool tip text
			SlidersHandler.lastSelection.hideToolTip();
		},
	//Manage the mousedown event (begin of drag)
	onmousedown		: function(e){
			if (!e)e=window.event;
			//Set the latest selection
			SlidersHandler.lastSelection=this.slider;	
					
			//Attach the event on slider 
			var s=SlidersHandler.lastSelection;
			s.container.onmousemove=SlidersHandler.onmousemove;
			s.container.onmouseup=SlidersHandler.onmouseup;
								
			//Start the drag event 
			SlidersHandler._dragData.screenX=e.screenX;
			SlidersHandler._dragData.screenY=e.screenY;
			SlidersHandler._dragData.dx=e.screenX - s.cursor.offsetLeft;
			SlidersHandler._dragData.dy=e.screenY - s.cursor.offsetTop;
			SlidersHandler._dragData.startValue=s.getValue();
		},
	//Manage the mousemove event (drag)
	onmousemove		: function(e){
			...
			//Get the positions
			var boundSize = s._range.getMaximum() - s._range.getMinimum();
			var size, pos, reset;
			switch(s._orientation){
					case eOrientation.horizontal :
						size = s.container.offsetWidth - s.cursor.offsetWidth;
						pos = e.screenX - SlidersHandler._dragData.dx;
						reset = Math.abs(e.screenY - SlidersHandler._dragData.screenY) > 100;
						break;
						
					case eOrientation.vertical :
						size = s.container.offsetHeight - s.cursor.offsetHeight;
						pos = s.container.offsetHeight - s.cursor.offsetHeight -(e.screenY - SlidersHandler._dragData.dy);
						reset = Math.abs(e.screenX - SlidersHandler._dragData.screenX) > 100;					
						break;
				}
			//Calculate the new value
			v=reset ? (SlidersHandler._dragData.startValue):(((s._range.getMinimum() + boundSize) * pos) / size);
				
			//Set the value
			s.setValue(v);
			SlidersHandler._cancelClickEvent=true;
		},
	//Manage the mouseup event (drop)
	onmouseup		: function(e){
			...
			//Detach events
			s.container.onmousemove=null;
			s.container.onmouseup=null;
			
			//Remove the cancel click
			setTimeout("SlidersHandler._cancelClickEvent=false;",50);
		},
	//Manage the onclick event to move the slider with click
	onclick			: function(e){
			if (!e)e=window.event;
			
			if (SlidersHandler._cancelClickEvent){
					SlidersHandler._cancelClickEvent=false;
					return;
				}
			
			var s=SlidersHandler.lastSelection;
			//Calculate the value according to the position of mouse			
			var contSize=(s._orientation==eOrientation.horizontal)?s.container.offsetWidth:s.container.offsetHeight;
			var curSize=(s._orientation==eOrientation.horizontal)?s.cursor.offsetWidth:s.cursor.offsetHeight;
			var maxSize=(s._orientation==eOrientation.horizontal)?(contSize - curSize):(contSize - curSize - (contSize - curSize));
			var boundSize=(s._range.getMaximum() - s._range.getMinimum());
			var mousePos=(s._orientation==eOrientation.horizontal)?e.screenX:e.screenY;
			var v=((boundSize * (mousePos / maxSize)) + s._range.getMinimum()) -1;
			s.setValue(v);
			
			SlidersHandler._cancelClickEvent=false;		
		},
	//Manage the keyDown event to move slider with keyboard
	onkeydown		: function(e){
			...
			switch (e.keyCode) {
				...
				case KEY_PAGEDOWN:	// page down
					...
					break;
				case KEY_END:	// end
					s.setValue(s._orientation==eOrientation.horizontal?s._range.getMaximum():s._range.getMinimum());
					break;
				case KEY_HOME:	// home
					s.setValue(s._orientation==eOrientation.horizontal?s._range.getMinimum():s._range.getMaximum());
					break;
				case KEY_UP:	// up (see KEY_RIGHT)
				case KEY_RIGHT:	// right
					s.setValue(s.getValue() + s.unitIncrement);
					break;

				case KEY_LEFT:	// left (see KEY_DOWN)
				case KEY_DOWN:	// down
					s.setValue(s.getValue() - s.unitIncrement);
					break;
			}
			
			//Don't propage the events
	...
	//Manage the events propagation of keyboard
	...
	//Manage the button down (doScroll)
	onscrolldown	: function(e){
			...
			SlidersHandler._scrollDirection=eScroll.down;
			SlidersHandler.doScroll(e);
		},
	//Manage the button up (doScroll)
	onscrollup		: function(e){
			...
			SlidersHandler._scrollDirection=eScroll.up;
			SlidersHandler.doScroll(e);	
		},
	//Scroll the slider with button
	doScroll		: function(e){
			if (!e)e=window.event;
			if (!SlidersHandler.lastSelection)return;
			SlidersHandler._cancelClickEvent=true;
			...
			//Set the value according to up/down direction of scrolling
			v=s.getValue();
			if (SlidersHandler._scrollDirection==eScroll.up){
					v +=s.blockIncrement;
				}
			else {
					v -=s.blockIncrement;
			...
			//Recursive 
			SlidersHandler.__scrollTimer=window.setTimeout("SlidersHandler.doScroll()",150);
		},
	//Stop the scroll on slider
	cancelScroll	: function(e){
			window.clearTimeout(SlidersHandler.__scrollTimer);
			window.setTimeout("SlidersHandler._cancelClickEvent=false;",50);
		}		
	};	

/* ===
	SliderSkin Object. Manage the style of sliders
=== */
function SliderSkin(skinName){
		this.name=skinName || eSkinName.classical;
		this.type=eSliderType.slider;
		this.fileName=this.name + '/' + this.name + '.' + EXTENSION_CSS;
		//Corrected the type of slider for the scroll bars
		switch(this.name){
				case eSkinName.blueCurveScroll: 
				case eSkinName.winXp: 	
					this.type=eSliderType.scrollBar;
					break;
			}
	}

/* ===
	Slider Object. Manage the sliders
===	*/
// CONSTRUCTOR 
function Slider(oQuestion,skinName,orientation) {	
	//Manage errors in args and the type of question
	if (!oQuestion)return;
	if (oQuestion.isClosed && !oQuestion.isUseList)return;
	if (!oQuestion.isClosed && oQuestion.type!=eQuestionType.numeric)return;
	
	//Set the id of slider
	this.id=oQuestion.id;
	
	//Set the question of slider
	this.question=oQuestion;
	
	//Set the style of slider
	this.skin=new SliderSkin(skinName);
	this._orientation = orientation || eOrientation.horizontal;
	this._isStyleSheetReady=false;
	
	//Import the stylesheet of slider
	AskiaScript.addStyleSheet(AskiaScript.pathOfScripts + 'v' + VERSION_OF_SCRIPTS + AskiaScript.pathSeparator + AskiaScript._CSSFolderName + AskiaScript.pathSeparator + this.skin.fileName);
	
	//Search all HTMLElement for the slider
	var _paNode=this.question.childNodes[0].htmlControl.parentNode;
	if (_paNode){
			var isOk=false;
			var _arrAvailableContainer=["DIV","SPAN","TD"];
			for (var i=0;i<_arrAvailableContainer.length;i++){
					if (_paNode.tagName==_arrAvailableContainer[i]){
							isOk=true;
							break;
						}
				}
			//The input control have not an available container
			if (!isOk)return false;
		}
	
	//Set the predefined html elements of slider
	this.container = _paNode;
	this.input	   =  this.question.childNodes[0].htmlControl;
	this.output	   =  document.getElementById(eResponsePrefix.sliderOutput + this.question.id) || null;

	//Add range of available values
	var _min, _max,_value;
	if (this.question.isClosed){
			_min=0;
			_max=this.question.childNodes[0].htmlControl.options.length-1;
			_value=null;
			for (var i=0;i<this.question.childNodes[0].htmlControl.options.length;i++){
					if(this.question.childNodes[0].htmlControl.options[i].value==this.question.childNodes[0].htmlControl.value){
							_value=i;
							break;
						}
				}
			...
			_min=this.question.min || 0;
			_max=this.question.max || 100;
			_value=this.input.value || _min;
		}
	this._range=new SliderRange(_min,_max,_value);
	
	//Increments
	var _bI=parseInt((_max - _min) / 10);
	if (_bI<1)_bI=1;
	this.blockIncrement = _bI;				//Increment when mousewheel, pgUp, pgDown ....
	this.unitIncrement  = 1;				//Unit of increment	
		
	//Create the htmlElements of slider
	this._create();
			
	//Add current sliders in the SliderHandler collections
	SlidersHandler.add(this);
}

//Create the HTMLElements of slider
Slider.prototype._create=function(){
    var _THIS=this;
    //Since v1.0.1 (Verify that the css was loaded before to create the slider
    if (!this._isStyleSheetAvailable()){
	...
	this.cursor = this.document.createElement("DIV");
		this.cursor.className = eSliderClassName.cursor;
	...
	//Create an tool tip box
	this.toolTip =new ToolTipText();
		
	//Define the behaviour on HTMLElement
	this.container.onclick		= SlidersHandler.onclick;
	this.container.onmousedown	= SlidersHandler.onmousedown;
	this.container.onmouseover	= SlidersHandler.onmouseover;
	this.container.onmouseout	= SlidersHandler.onmouseout;
	this.container.onkeydown	= SlidersHandler.onkeydown;
	this.container.onkeypress	= SlidersHandler.onkeypress;
	this.container.onmousewheel	= SlidersHandler.onmousewheel;
	
	//Manage the roll over on cursor
	this.cursor.onmouseover		= function(e){this.className = eSliderClassName.cursorOver;};
	this.cursor.onmouseout		= function(e){this.className = eSliderClassName.cursor;};
	
	//Manage the selection
	this.cursor.onselectstart	= this.container.onselectstart = function () { return false; };
		
	//Behaviour for scollbar
	if (this.skin.type==eSliderType.scrollBar){
			this.minSide.onmousedown	= SlidersHandler.onscrolldown;
			this.maxSide.onmousedown	= SlidersHandler.onscrollup;
			this.minSide.onmouseup		= SlidersHandler.cancelScroll;
			this.maxSide.onmouseup		= SlidersHandler.cancelScroll;
		}
	//Attached function in the onchange event of range
	_THIS = this;
...
//(since v1.0.1) Verify that the stylesheet is not empty
Slider.prototype._isStyleSheetAvailable=function(){ 
        if (this._isStyleSheetReady)return true;
        //Search the specific rules into the stylesheets
        var sSearchRule="." + this._orientation + "-" + this.skin.name;
            sSearchRule =sSearchRule.toLowerCase();
        for (var nSheet=0;nSheet<document.styleSheets.length;nSheet++){
                var myRules=null;
                try {
                    myRules =(document.styleSheets[nSheet].rules)?document.styleSheets[nSheet].rules:document.styleSheets[nSheet].cssRules;
                }
                catch(ex){/*Do nothing*/}
                if (!myRules)continue;
                for (var nRules=0;nRules<myRules.length;nRules++){
                        if (myRules[nRules].selectorText.toLowerCase()==sSearchRule){
                                this._isStyleSheetReady=true;
                                return true;
                               }
                    }
            }
...
Slider.prototype._recalculate = function() {
		var contw = this.container.offsetWidth;
		var conth = this.container.offsetHeight;
		var curw = this.cursor.offsetWidth;
		var curh = this.cursor.offsetHeight;
		var barw = this.bar.offsetWidth;
		var barh = this.bar.offsetHeight;

		switch(this._orientation){
				case eOrientation.horizontal : // Border-box layout for horizontal slider
					maxWidth=(contw - curw);
					realValue=(this.getValue() - this._range.getMinimum());
					sizeOfRange=(this._range.getMaximum() - this._range.getMinimum());
							
					//Calculate the left position of cursor
					this.cursor.style.left = (maxWidth * (realValue / sizeOfRange)) + "px"; 
					//Place cursor in middle
					this.cursor.style.top = ((conth - curh)/2) + "px";						
					
					//Place the tool tip over the cursor
					if (this.toolTip){
						var ttTSpace=(conth<20)?10:0;
						var ttT=this.container.offsetTop - (conth + ttTSpace) + "px";
						var ttL=this.container.offsetLeft + parseInt(this.cursor.style.left) + "px";
						this.toolTip.move(ttL,ttT);
					}
										
					//Place the bar in middle
					this.bar.style.top = (conth - barh) / 2 + "px";
					//Calculate the left position of bar
					this.bar.style.left = curw / 2 + "px";
					//Set border of slider
					this.bar.style.width = Math.max(0, contw - curw - 2)+ "px";
					this.bar.firstChild.style.width = Math.max(0, contw - curw - 4)+ "px";				
					break;
					
				case eOrientation.vertical : //Border-box layout for vertical slider				
					maxHeight=conth - curh;
					realValue=(this.getValue() - this._range.getMinimum());
					sizeOfRange=(this._range.getMaximum() - this._range.getMinimum());
				
					//Place cursor in center 
					this.cursor.style.left = ((contw - curw) / 2) + "px";
					//Calculate the top position of cursor
					this.cursor.style.top = maxHeight - (maxHeight * (realValue / sizeOfRange)) + "px";

					//Place the tool tip over the cursor
					if (this.toolTip){
						var ttT=this.container.offsetTop + parseInt(this.cursor.style.top) + "px";
						var ttLSpace=(contw<20)?15:5;
						var ttL=this.container.offsetLeft + (contw + ttLSpace)+ "px";
						this.toolTip.move(ttL,ttT);
					}

					//Place the bar in center
					this.bar.style.left = ((contw - barw) / 2) + "px";
					//Calculate the top position of bar
					this.bar.style.top = (curh / 2) + "px";
					//Set border of slider 
					this.bar.style.height = Math.max(0, conth - curh - 2) + "px";	
					this.bar.firstChild.style.height = Math.max(0, conth - curh - 4) + "px";			
					break;
			}
	};
...
Slider.prototype.showToolTip=function(){
		//Set the text on tooltip
		if (this.toolTip){
				var t=(!this.question.isUseList)?this._range.getValue():this.input.options[this._range.getValue()].text;
				this.toolTip.setText(t);
...
Slider.prototype.setValue = function (value) {
		...
		v=(this.question.isUseList)?this.input.options[this._range.getValue()].value:this._range.getValue();
		this.input.value=v;
		if (this.output)this.output.innerHTML=(!this.question.isUseList)?this._range.getValue():this.input.options[this._range.getValue()].text;
		this.showToolTip();
	};
...
Slider.prototype.onchange=function(){/*RaiseEvent*/};


/* ===
	SliderRange object, to manage the available values on slider
=== */ 
//Constructor
function SliderRange(min,max,value) {
		this._value = value || 0;		//Actual value
		this._minimum = min || 0;		//Minumum value
		this._maximum = max || 100;		//Maximum value
	}
...
SliderRange.prototype.setValue = function (value) {
		...
		if (value > this._maximum) this._value = this._maximum;
...
﻿/*global ActiveXObject, AskiaScript, Calculation, CalculationHandler, Calendar, CustomControl, CustomControlBehaviour, eCustomControlState, eOrientation, ePosition, eRankingType, ErrorStack, eSkinName, FieldValidator, filterBox, filterBoxHandler, getDateToString, getPosition, isIE, Slider, replace */
/*jshint evil:true,bitwise:false,proto:true,nomen:false,strict:false,globalstrict:false,onevar:false,white:false,plusplus:false,eqeq:false*/
/*------------------------------------------------------------------------------\
...
|-------------------------------------------------------------------------------|
| Created 2006-05-23 | All changes are in the log above. | Updated 2014-02-04	|
|-------------------------------------------------------------------------------|
|														All rights reserved		|
|Askia - Common - API Version 1.2.0			Copyright Askia (C) 1994-2014		|
\-------------------------------------------------------------------------------*/
...
var QuestionHandler = {
	...
	//Indicates if the element is a slider
	// Since 1.2.0 used the class askia-slider
	isSlider							: function (htmlElt) {
		if (htmlElt.getAttribute(eHTMLAttributes.isSlider)) {
			return true;
		}
		if ($(htmlElt).hasClass(eAskiaClass.slider)) {
			return true;
		}
		return false;
	},

	//Get the skin name of the slider
	// Since 1.2.0 used the class
	getSliderSkinName					: function (htmlElt) {
		if (htmlElt.getAttribute(eHTMLAttributes.skinName)) {
			return htmlElt.getAttribute(eHTMLAttributes.skinName);
		}
		var item;
		for (item in eSkinName) {
			if (eSkinName.hasOwnProperty(item)) {
				if ($(htmlElt).hasClass(eSkinName[item]) || $(htmlElt).hasClass(item)) {
					return item;
				}
			}
		}
...
function Question(id,shortcut,caption,type,isAllowDk,isLive,min,max,defaultValue,decimalNumber){
		...
		//Slider layout
		this.sliderSkinName=null;			//eSkinName of slider
		this.sliderOrientation=null;		//eOrientation of slider
...
Question.prototype.init=function(){
	...
	for (var i=0;i<arrInput.length;i++){
			...
			//Set semi-open if the question is closed
			if (this.isClosed && arrInput[i].getAttribute("type")=='text' && arrInput[i].getAttribute("name").indexOf(eResponsePrefix.ranking)==-1)this.isSemiOpen=true;
			//Set ranking if the question have the ranking style (textbox)
			if (this.isClosed && arrInput[i].getAttribute("type")=='text' && arrInput[i].getAttribute("name").indexOf(eResponsePrefix.ranking)!=-1){
					this.isRanking=true;
					this.isSorted=true;
			...
			if (!r.virtualControl){
					...
					if (this.isRanking && r.htmlControl.getAttribute("type")=="text" && r.htmlControl.value!=''){
...
Question.prototype.add = function (oResponse) {
	...
	if (this.type == eQuestionType.date) {
		...
		// this.all[oResponse.id].calendar = new Calendar(this.id, this.min, this.max, this.defaultValue);
	...
	//Create slider
	if ((this.type == eQuestionType.single || this.type == eQuestionType.numeric) && QuestionHandler.isSlider(oResponse.htmlControl)) {
		//Search the skin
		this.sliderSkinName = QuestionHandler.getSliderSkinName(oResponse.htmlControl);
		if (!this.sliderSkinName) this.sliderSkinName = null;
		if (this.sliderSkinName) this.sliderSkinName = eSkinName[this.sliderSkinName];

		//Search the orientation
		this.sliderOrientation = oResponse.htmlControl.getAttribute(eHTMLAttributes.sliderOrientation);
		if (this.sliderOrientation == "") this.sliderOrientation = null;
		if (this.sliderOrientation) this.sliderOrientation = eOrientation[this.sliderOrientation];

		this.all[oResponse.id].slider = new Slider(this, this.sliderSkinName, this.sliderOrientation);
		if (!this.all[oResponse.id].slider.id) this.all[oResponse.id].slider = null;
	}
...
Question.prototype._getIdOfResponse=function(htmlControl){
	...
	if (htmlControl.getAttribute("type")=='text')sId=htmlControl.getAttribute("name");
...
function Response(id,caption,htmlControl){
	...
	this.slider=null;				//Slider control
	this.calendar=null;				//Calendar object
...
Response.prototype._attachEvents=function(){
	...
	//Manage event on slider
	if (this.slider){
			this.slider._response=this;
			if (this.parentNode.isUseList){
					this.slider.onchange=QuestionHandler.select;
				}
			else {
					this.slider.onchange=QuestionHandler.blur;
				}
		}
			
	//Manage event on calendar
	if (this.parentNode.type==eQuestionType.date && this.calendar){
			var _THIS=this;
			this.calendar.onchange=function(){
				_THIS.htmlControl.value=getDateToString(_THIS.calendar.date);
				_THIS.calendar.hide();
				//Fires the event on change
				_THIS.parentNode._onchange();
			};
		}
	...
	if (sType.toLowerCase()=='radio' || sType.toLowerCase()=='checkbox'){
		...
		if (sType.toLowerCase()=='text' || this.htmlControl.tagName=='TEXTAREA'){
...
Response.prototype.click = function (e) {
	...
	if (this.parentNode.type == eQuestionType.date && this.uiControl) {
		...
		//Added pixels to corrected the position of calendar
		var tAddedPx=(!isIE)?10:15;	
		var lAddedPx=8;
						
		//Calculate the position of calendar
		var t=getPosition(this.virtualControl,ePosition.top) + this.virtualControl.offsetHeight + tAddedPx;
		var l=getPosition(this.virtualControl,ePosition.left) + lAddedPx;
						
		//Show the calendar
		this.calendar.show(l,t);
														
		//Manage the lost focus
		var _THIS=this;
		this.calendar.onblur=function(){
		_THIS.calendar.hide();
		};
		}
		else {
...
//Events
Calculation.prototype.onExecute=function(htmlControl,result){/* RaiseEvent */};﻿/*global ActiveXObject, AskiaScript, eQuestionType, ErrorStack, QuestionHandler, replace */
/*jshint evil:true,bitwise:false,proto:true,nomen:false,strict:false,globalstrict:false,onevar:false,white:false,plusplus:false,eqeq:false*/
/*----------------------------------------------------------------------------\
|							Askia Flash Scripts								  |
|-----------------------------------------------------------------------------|
...
|							(http://www.askia.com)							  |
|-----------------------------------------------------------------------------|
| This file will manage the integration of flash							  |
|-----------------------------------------------------------------------------|
|							Copyright Askia © 1994-2010						  |
|-----------------------------------------------------------------------------|
| Dependencies:																  |
...
| 2009-05-06 |V 1.0.0														  |
|			 |+ FIRST VERSION												  |
|			 |	> Manage the get/set to flash object						  |
|-----------------------------------------------------------------------------|
| 2010-02-19 |V 1.1.4														  |
|			 |+ For flash send init the itemCaption when there is one item in |
|			 | the loop														  |
...
|-----------------------------------------------------------------------------|
| 2010-06-02 |V 1.1.5														  |
|			 |+ Use the SWFObject 2.2										  |
|-----------------------------------------------------------------------------|
| 2010-12-20 |V 1.1.9														  |
|			 |+ Send the "Don't know" response of open-ended and numeric		  |
|-----------------------------------------------------------------------------|
| Created 2009-05-06 | All changes are in the log above. | Updated 2010-12-20 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
|Askia - Flash - API Version 1.1.9				Copyright Askia © 1994-2010   |
\----------------------------------------------------------------------------*/
...
///<summary>
///Default width of flash object
///</summary>
var FLASH_DEFAULT_WIDTH=600;
///<summary>
///Default height of flash object
///</summary>
var FLASH_DEFAULT_HEIGHT=500;
///<summary>
///Default version of flash object
///</summary>
var FLASH_DEFAULT_VERSION='9.0.0';
///<summary>
///Allow an express installation of flash component when it's not exist
///</summary>
var FLASH_EXPRESS_INSTALL_URL=false;
///<summary>
///Delimiter of index
///</summary>
var FLASH_INDEX_DELIMITER="#index:";

///<summary>
///Util to manage the flash integration
///</summary>
var AskiaFlashUtil = {

	///<summary>
	///Creates the flash object into the html document
	///</summary>
	///<param name="containerId">Id of HTMLContainer</param>
	///<param name="shortcuts">Shortcuts of question (can be "", a single shortcut or an array of shortcuts ["q1","q2"])</param>
	///<param name="url">Url of flash object</param>
	///<param name="width">Width of flash object</param>
	///<param name="height">Height of flash object</param>
	///<param name="additionalFlashVars">Additional flash variables (Object with pair name/value)</param>
	///<param name="additionalParameters">Additional parameters (Object with pair name/value)</param>
	///<param name="additionalAttributes">Additional attributes (Object with pair name/value)</param>
	createFlash: function (containerId, shortcuts, url, width, height, additionalFlashVars, additionalParameters, additionalAttributes) {
		var strId = replace(containerId, ' ', '_');

		var strShortcuts = (shortcuts == "") ? "" : (shortcuts.length && shortcuts.join) ? this._getShortcutsParamFromArray(shortcuts) : this._getShortcutsParam(shortcuts);

		var flashvars = {
			askiaFlashId: strId,
			askiaShortcuts: strShortcuts
		};
		//Adding additional flash variables
		if (additionalFlashVars) {
			for (var item in additionalFlashVars) {
				flashvars[item] = additionalFlashVars[item];
			}
		}
		...
		// Fix: Issue on Safari with the wmode=transparent;
		var ua = navigator.userAgent.toLowerCase(),
			isSafari = (ua.indexOf('safari') != -1 && ua.indexOf('chrome') === -1);

		if (isSafari) {
			delete additionalParameters.wmode;
		}

		//Adding additional parameters
		if (additionalParameters) {
			for (var item in additionalParameters) {
				params[item] = additionalParameters[item];
			}
		}
		var attributes = {
			id: strId,
		...
		if (additionalAttributes) {
			for (var item in additionalAttributes) {
				attributes[item] = additionalAttributes[item];
			}
		}
		var w = width || FLASH_DEFAULT_WIDTH;
		var h = height || FLASH_DEFAULT_HEIGHT;
		document.getElementById(containerId).className += " askia-unselect-flash";
		swfobject.embedSWF(url, containerId, w, h, FLASH_DEFAULT_VERSION, FLASH_EXPRESS_INSTALL_URL, flashvars, params, attributes);
	},
	///<summary>
	///Returns a shortcut to send to the flash side
	///</summary>
	///<param name="shortcut">Shortcut of question</param>
	_getShortcutsParam: function (shortcut) {
		var arrLoopIndex = shortcut.split(FLASH_INDEX_DELIMITER);
		var q = QuestionHandler.getQuestionsByShortcut(arrLoopIndex[0]);
		if (!q) return "";
		if (!q.length || arrLoopIndex.length > 1) return shortcut;
		var LENGTH_SEPARATOR = "-ASKIA-LENGTH:";
		return shortcut + LENGTH_SEPARATOR + q.length;
	},
	///<summary>
	///Build the parameter of shortcuts using the array
	///</summary>
	///<param name="shortcuts">Array of shortcuts</param>
	_getShortcutsParamFromArray: function (shortcuts) {
		var strReturn = "";
		var SHORTCUT_DELIMITER = "-ASKIA-DEL-";
		for (var i = 0; i < shortcuts.length; i++) {
			if (strReturn != "") strReturn += SHORTCUT_DELIMITER;
			strReturn += this._getShortcutsParam(shortcuts[i]);
		}
		return strReturn;
	},
	///<summary>
	///Send the information of question
	///<summary>
	///<param name="flashId">Id of flash object</param>
	///<param name="shortcut">Shortcut of question</param>
	///<param name="index">Index of question</param>
	getQuestionInfo: function (flashId, shortcut, index) {
		var idx = index || 0,
				qs = QuestionHandler.getQuestionsByShortcut(shortcut),
				q = (qs.length) ? qs[idx] : qs,
				rs = [],
				strValue = '',
				strItemCaption = '',
				i, l, el, rr, r, oQ,
		...
		// Build responses
		for (i = 0, l = q.childNodes.length; i < l; i += 1) {
			rr = q.childNodes[i];
			//Creates a smaller object with the summary of real responses
			r = {
				id: rr.id,
				index: rr.index,
				isSelected: (q._selRespById[rr.id]) ? true : false,
				caption: rr.caption,
				isExclusive: rr.isExclusive,
				isSemiOpenBox: rr.isSemiOpenBox,
				value: (rr.isSemiOpenBox || (rr.htmlControl.tagName.toLowerCase() === 'textarea' || rr.htmlControl.getAttribute("type").toLowerCase() === 'text')) ? rr.htmlControl.value : ""
			};
			...
			if (r.value && (q.type === eQuestionType.numeric || q.type === eQuestionType.open)) {
				strValue = r.value;
			}
			rs[rs.length] = r;
		}
		...
		// Search the item caption
		if (!q.isClosed) { // Try to search in cpt element first when open-ended or numeric in loop
			el = QuestionHandler.getCaptionElement(q.id, 0);
		} else {
			// QuestionHandler.getColumnCaptionElement
			el = QuestionHandler.getQuestionCaptionElements(q.id);
			if (el && $.isArray(el)) {
				el = el[0];
			}
		}
		...
		if (el) {
			strItemCaption = $(el).text();
		}

		if (strItemCaption === '' && q.questionLabel && q.questionLabel.length) {
			strItemCaption = $(q.questionLabel[0]).text();
		}

		// Creates a smaller object with the summary of question
		oQ = {
			type: q.type,
			caption: q.caption,
			itemCaption: strItemCaption,
			min: q.min,
			max: q.max,
			value: strValue,
			allowDk: q.isAllowDk
		...
		// Creates an array with the error messages
		err = [];
		errStckItem = ErrorStack.all[q.id];
		if (errStckItem) {
			err = errStckItem.childNodes;
		}

		// Send details to the flash object
		obj = (window[flashId]) ? window[flashId] : document[flashId];
		if (obj && typeof obj.setQuestionInfo === 'function') {
			obj.setQuestionInfo(shortcut, idx, oQ, rs, err);
		}
	}
};﻿/*global ActiveXObject, AskiaScript, replace */
/*jshint evil:true,bitwise:false,proto:true,nomen:false,strict:false,globalstrict:false,onevar:false,white:false,plusplus:false,eqeq:false*/
...
};﻿/*global ActiveXObject, AskiaScript, eQuestionType, eAskiaClass, QuestionHandler, ePosition, getPosition, ErrorStack, eErrorDisplayMode, isIE, replace */
/*jshint evil:true,bitwise:false,proto:true,nomen:false,strict:false,globalstrict:false,onevar:false,white:false,plusplus:false,eqeq:false*/
/*----------------------------------------------------------------------------\
...
|-----------------------------------------------------------------------------|
| Created 2007-09-20 | All changes are in the log above. | Updated 2011-11-21 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
| WebProd - Ranking - API Version 1.2.0	    	Copyright Askia © 1994-2011   |
\----------------------------------------------------------------------------*/
...
RankingDragDrop.prototype._writeZones=function(){
	...
	var drgT=parseInt(this.dragZone.style.top);
		if (isNaN(drgT)){
		        drgT=getPosition(this.dragZone,ePosition.top) + 20;
		        this.dragZone.style.top=drgT;
		    }
	var drgL=parseInt(this.dragZone.style.left);
		if (isNaN(drgL)){
		        drgL =getPosition(this.dragZone,ePosition.left) + 15;
	...
	if (isNaN(drpT)){
		    drpT =getPosition(this.dropZone,ePosition.top) + 20;
		    this.dropZone.style.top=drpT;
		}
	var drpL=parseInt(this.dropZone.style.left);
	if (isNaN(drpL)) {
		    drpL =getPosition(this.dropZone,ePosition.left) + 15;
...
/*jshint evil:true,bitwise:false,proto:true,nomen:false,strict:false,globalstrict:false,onevar:false,white:false,plusplus:false,eqeq:false*/
/*----------------------------------------------------------------------------\
|							Askia ToolTipText Objects						  |
|-----------------------------------------------------------------------------|
|                          Created by Askia Company							  |
|							(http://www.askia.com)							  |
|-----------------------------------------------------------------------------|
| Display a ToolTip text													  |
|-----------------------------------------------------------------------------|
|							Copyright Askia © 1994-2006						  |
|-----------------------------------------------------------------------------|
| Dependencies:																  |
...
| 2006-06-07 |V 1.0.0														  |
|			 |+ First version												  |
|			 |+ Creation and position of tool tip text						  |
|-----------------------------------------------------------------------------|
| 2009-10-27 |V 1.1.2														  |
|			 |+ Creates container on fly									  |
|-----------------------------------------------------------------------------|
| Created 2006-06-07 | All changes are in the log above. | Updated 2009-10-27 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
|WebProd - ToolTipText - API Version 1.1.2		Copyright Askia © 1994-2009   |
\----------------------------------------------------------------------------*/
...
===	*/
//ClassName of HTMLElements
var eToolTipClassName={
		container		: 'ToolTip-Container',
		opacity			: 'ToolTip-Opacity-'
	};

/* ===
	ToolTipTextHandler. Manage the collections of tool tip
=== */
var ToolTipTextHandler={
		//Collections of ToolTipText objects
		all			: {},
		childNodes	: [],
		lastSelection	: null,
		
		//Get new id for ToolTip
		getNewId	: function(){
				return '-askia-ttp-' + this.childNodes.length;
			},
		//Add ToolTipText in collection
		add			: function(oToolTip){
				//Don't add a duplicate ToolTip
				if (this.all[oToolTip.id])return this.all[oToolTip.id];
				//Add the ToolTip into collections
				this.all[oToolTip.id]=oToolTip;
				this.childNodes[this.childNodes.length]=this.all[oToolTip.id];
				return this.all[oToolTip.id];
			}
	};

/* === 
	ToolTipText. Manage the ToolTip
=== */
function ToolTipText(text){
		//Get a newest id
		this.id=ToolTipTextHandler.getNewId();
		//Set the text of tool tip
		this._text=text || '';
		//Create the HTMLElement
		this.container=null; //this._createContainer();
		//State of tool tip
		this._isVisible=false;
		//State of opacity
		this._opacity=10;
		//Add them into collection
		ToolTipTextHandler.add(this);
	}
	
//Create and return the HTMLElement which represent the container
//of tool tip text
ToolTipText.prototype._createContainer=function(){
	var elt=document.createElement("DIV");
		elt.id=this.id;
		elt.className=eToolTipClassName.container;
		elt.style.visibility="hidden";
		elt.innerHTML=this._text;
	//if (!isIE)
		document.body.appendChild(elt);
	/*else 
		document.write(elt.outerHTML);*/
	return document.getElementById(this.id);
};
	
//Set the text into the container
...
ToolTipText.prototype.getText=function(){
	return this._text;
};
//Display the ToolTip
ToolTipText.prototype.display=function(left,top){
	if (!this.container)this.container=this._createContainer();
	if (ToolTipTextHandler.lastSelection)ToolTipTextHandler.lastSelection.hide();
	if (left || left==0)this.container.style.left=left;
	if (top || top==0) this.container.style.top=top;
	this._isVisible=true;
	this._opacity=10;
	this.container.className=eToolTipClassName.container;
	this.container.style.visibility="visible";
	ToolTipTextHandler.lastSelection=this;
};
//Hide the Tooltip
ToolTipText.prototype.hide=function(){
	this._isVisible=false;
	this._opacity=10;
	this._setOpacityToHide();
};
//Manage the opacity
ToolTipText.prototype._setOpacityToHide=function(){
	if (this._isVisible)return;
	if (!this.container)this.container=this._createContainer();
	this._opacity -=1;
	if (this._opacity<=0 && !this._isVisible){
			this.container.style.visibility="hidden";
			return;
		}		
		
	this.container.className =eToolTipClassName.container + ' ' + eToolTipClassName.opacity + this._opacity;

	//Recursivity
	var _THIS=this;
	window.setTimeout(function(){
			_THIS._setOpacityToHide();
		},75);
};
//Move the ToolTip
ToolTipText.prototype.move=function(left,top){
	if (!this.container)this.container=this._createContainer();
	if (left || left==0)this.container.style.left=left;
	if (top ||top==0) this.container.style.top=top;
};
...
|-----------------------------------------------------------------------------|
| Created 2006-05-29 | All changes are in the log above. | Updated 2007-05-29 |
|-----------------------------------------------------------------------------|
|														All rights reserved	  |
|Askia - Navigator - API Version 1.1.0			Copyright Askia © 1994-2007   |
\---------------------------------------------------------------------------- */
